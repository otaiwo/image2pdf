...
const STATUS_STYLES: Record<JobStatus, StatusStyle> = {
    completed: {
        ...
        icon: CheckCircle2,
        color:
            "text-green-600 bg-green-50 border-green-200 dark:bg-green-900/10 dark:border-green-900/30",
    ...
    processing: {
        ...
        icon: Loader2,
        color:
            "text-blue-600 bg-blue-50 border-blue-200 dark:bg-blue-900/10 dark:border-blue-900/30",
    ...
    pending: {
        ...
        icon: Clock,
        color:
            "text-yellow-600 bg-yellow-50 border-yellow-200 dark:bg-yellow-900/10 dark:border-yellow-900/30",
        text: "Pending",
    },
    ...
    failed: {
        ...
        icon: XCircle,
        color:
            "text-red-600 bg-red-50 border-red-200 dark:bg-red-900/10 dark:border-red-900/30",
...
const getStatusStyle = (status: string): StatusStyle =>
    STATUS_STYLES[status as JobStatus] ?? STATUS_STYLES.pending;
...
export const JobSidebar: React.FC<JobSidebarProps> = ({
    ...
    return (
        <div className="w-full space-y-6">
            {activeJob && (() => {
                const style = getStatusStyle(activeJob.status);
                ...
                return (
                    <>
                        <div
                            className={`border-2 rounded-2xl p-6 transition-colors ${style.color}`}
                        >
                            ...
                            <div className="space-y-3 mb-6">
                                ...
                                <div className="flex justify-between items-end">
                                    ...
                                    <span
                                        className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${style.badge}`}
                                    >
                                        {activeJob.status}
                                    </span>
                                </div>
                                ...
                                >
                                    <div className="flex flex-col min-w-0 flex-1">
                                        ...
                                        <div className="flex items-center gap-2 mt-1.5">
                                            <span
                                                className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${style.badge}`}
                                            >
                                                {job.status}
                                            </span>
                                    ...
                                    {job.status === "completed" && (
                                        <button
                                            aria-label={`Download PDF for ${job.filename}`}
                                            onClick={() =>
                                                onDownload(job)
                                            }
